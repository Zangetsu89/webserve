...
void	checkDirMethods(DirSettings D1, unsigned int server_number)
{
	...
	for (unsigned int j = 0; j < methods.size(); j++)
		if (methods[j]!= "GET" && methods[j]!= "POST" && methods[j]!= "DELETE")
			throw(std::invalid_argument("Server " + std::to_string(server_number) + " contains wrong allowed_methods."));	
}

void    checkConfigFile(Config C1)
{
	if (C1.getServers().size() < 1)
		throw(std::invalid_argument("No servers information info can be read from Config file"));
	std::vector<Server> Servers = C1.getServers();
	for (unsigned int i = 0; i < Servers.size(); i++)
	{
		if (Servers[i].getServerName().length() < 1)
			throw(std::invalid_argument("Server " + std::to_string(i) + " has no name"));
		if (Servers[i].getPorts().size() < 1)
			throw(std::invalid_argument("Server " + std::to_string(i) + " has no listening ports"));
		if (Servers[i].getRootDir().length() < 1)
			throw(std::invalid_argument("Server " + std::to_string(i) + " has no root directory path"));
		if (Servers[i].getRootDirSettings().getIndexPage().length() < 1)
			throw(std::invalid_argument("Server " + std::to_string(i) + " has no index page"));

...
Request::Request() : _sizeR(0), _requestBodyLength(0), _requestBodySentCGI(0){}
...
Request &Request::operator=(const Request &source)
{
	if (this != &source)
	{
		...
		_requestBodySentCGI = source._requestBodySentCGI;
...
int Request::getRequestBodySentCGI() const
{
	return (_requestBodySentCGI);
}
...
void Request::setRequest(std::vector<Server> *list_server, SocketConnect *socket)
{
	...
	catch (Exception_Request &e)
	{
		...
		std::cout << "Error num: " << e._error_num << "Starus num " << e._status_num << std::endl;
		_requestSocket->getSocketResponse()->setError(e._error_num);
		_requestSocket->getSocketResponse()->setStatus(e._status_num);
	}
...
void	Request::addRequestBodySentCGI(int size)
{
	_requestBodySentCGI += size;
}
